...
The CNN~+~WS fusion evaluated here is a faithful instance of the paper's Model~5
(Figure~\ref{fig:pipeline}): a training-fold-standardised concatenation of the CNN and WS
vectors, classified by the same Subspace k-NN ensemble. Read against the four implementation differences in Chapter~\ref{sec:results}, the
...
held-out AU-ROCs ($0.762$ and $0.816$) are. Razali's own Table~7 shows the same gap---mass CV exceeds test accuracy for the learned models ($100.0\%$/$83.3\%$ CNN, $98.0\%$/$91.7\%$ fusion) but not for WS-only ($69.0\%$/$83.3\%$)---read there as limited data, not the CV optimism identified here. The project therefore leads with held-out AU-ROC
