...
both tasks (Chapter~\ref{sec:results}); the project's substantive result lies beyond it,
in the mass extensions (Chapter~\ref{sec:extensions}), where preserving the
wavelet-scattering \emph{spatial map} --- rather than enriching a scalar descriptor ---
is the decisive lever. This chapter interprets the replication against the paper, draws
...
ensemble, with no GLCM branch. Its interpretive upshot, read against the four
implementation differences listed in Chapter~\ref{sec:results}, is that the paper's
central mechanism --- handcrafted scattering features rescuing a data-starved CNN --- is
reproduced \emph{exactly in the regime the paper invokes it, and only there}. On
...
training-free scattering features lifts the honest, leakage-free result: fusion (AU-ROC
$0.816$) outranks both CNN-only ($0.762$) and WS-only ($0.726$). This is the clearest
evidence in the project that fusion helps, and it reproduces the paper's qualitative claim
on the mass task while showing precisely why that claim does \emph{not} transfer to our
tissue pipeline.

The paper's headline mass result (CNN~+~WS, $91.7\%$ test, AU-ROC $1.00$) rests on a single
${\sim}25$-patch split: our cross-validation reproduces its ${\approx}98\%$, but the honest
repeated held-out estimate is lower and noisier ($0.816 \pm 0.057$). We affirm the qualitative
claim that fusion benefits the mass task; why the $1.00$ itself is not a stable target is taken
...
observation, however, is that this gap is closed \emph{and substantially exceeded} by
...
contrast operation. On \emph{mass}, by contrast, the same richer scalar descriptor gives no
gain ($0.734$ versus $0.726$) --- foreshadowing the central extension result below.

\section{What the extensions establish}

The extension experiments converge on one message: for the mass task, \emph{preserving
...
reported as a preprocessing-sensitivity result, not a change to the paper-style replication.

The contrast with tissue is the interpretive key. Tissue density is a texture problem that
a richer \emph{scalar} descriptor already solves ($0.984$), with no need to retain spatial
...
the mass side. The CNN and fusion methods support that
conclusion rather than supplant it: the region-aware late fusion is numerically strongest
...
best \emph{number} while the region-aware representation is the substantive \emph{result}.
...
only ${\sim}25$ patches, with no measure of variance. At that size a single misclassification
...
\textbf{Optimistic CV on learned features.} Cross-validation computed on \emph{learned} CNN
...
Three data-provenance issues surfaced during the project; each is documented in full in the
Step~0--2 notebooks, and none changes the qualitative conclusions.
...
The full audit is in the \textit{Step 0.3} notebook.
