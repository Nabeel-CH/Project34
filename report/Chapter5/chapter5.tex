...
\begin{table}[h]
\centering
\caption{Tissue task --- master method comparison across paper benchmarks, replication models, extensions, and selected negative controls. AU-ROC is the five-seed mean~$\pm$~standard deviation unless the row is a Razali paper benchmark.}
...
\begin{table}[h]
\centering
\caption{Mass task --- master method comparison across paper benchmarks, replication models, supporting extensions, region-aware/spatial-map models, and selected negative controls. AU-ROC is the five-seed mean~$\pm$~standard deviation unless the row is a Razali paper benchmark.}
...
\section{Scattering Covariance as the Tissue-Side Extension (Supervisor-Suggested)} \label{sec:scov_tissue}

\subsection*{Motivation}
The WS-only baseline (Section~\ref{sec:stage1}) trails the paper's reported result by roughly $7$~pp on the tissue task and, as later stages will show, gives a relatively weak mass-classification result. With strict paper-replication established as a reference point, the project supervisor recommended exploring the scattering covariance methodology of Cheng \& M\'enard~\cite{cheng2021scattering} as a richer texture representation. The supervisor explicitly warned against the alternative of simply flattening the full scattering tensor and applying PCA --- a non-canonical operation that would conflict with the mathematical construction of the scattering transform. We therefore investigated scattering covariance as an extension that:
\begin{itemize}
...
\begin{table}[h]
\centering
\caption{$L$ sweep at fixed $J=6$ for the adapted scattering covariance, tissue task, five-seed protocol. CV is the seed-34 group-aware mean; AU-ROC and test accuracy are five-seed means. Source: \texttt{scov\_JL\_ablation\_tissue.csv}.}
...
\begin{table}[h]
\centering
\caption{$J$ sweep at fixed $L=3$, tissue task, five-seed protocol. CV is the seed-34 group-aware mean; AU-ROC and test accuracy are five-seed means. Source: \texttt{scov\_JL\_ablation\_tissue.csv}.}
...
\caption{Tissue scattering-covariance ablation (\textit{FINAL Step~2.3}): seed-34 group-aware CV accuracy versus $L$ at fixed $J=6$ (left) and versus $J$ at fixed $L=3$ (right). CV peaks at $L=3$ and $J=6$; larger $L$ raises dimensionality and CV variance without improving accuracy.}
\label{fig:scov_JL}
\end{figure}
...
\begin{table}[h]
\centering
\caption{Mass task --- scattering covariance vs.\ averaged WS, five-seed protocol (AU-ROC and test accuracy mean~$\pm$~std; $115$ patches, $24$-patch held-out split). Source: \texttt{supporting\_extensions\_results.csv}.}
\label{tab:scov_mass}
\begin{tabular}{lccc}
\toprule
...
\begin{table}[h]
\centering
\caption{Mass-side $L$-control for scattering covariance at fixed $J=6$ (five-seed AU-ROC; negative control). No $L$ meaningfully beats the averaged-WS baseline of $0.726$. Source: \texttt{scov\_L\_ablation\_mass.csv}.}
...
\begin{itemize}
    ...
    \item respects the supervisor's no-flattening constraint by construction (the output is a scalar coefficient vector, not a flattened spatial tensor);
...
The project's main mass-side extension is to \emph{preserve the scattering spatial map and learn over it with a small CNN} (\textit{FINAL Step~2.4}). Its premise (Section~\ref{sec:why_extension}) is that averaged WS fails on mass because it discards \emph{where} lesion structure lies; rather than enrich a scalar descriptor --- as scattering covariance does for tissue --- this approach keeps the scattering \emph{maps} at a lower invariance scale and lets a small CNN learn over them. The lesion-relative \emph{region-aware} decomposition described below is the interpretable, morphology-conditioned instantiation of that idea; as the ablation will show, the measured accuracy comes mainly from preserving the map and learning over it, with the region split adding interpretability rather than a proven numerical gain.
...
\begin{table}[h]
\centering
\caption{Mass task: region-aware extension story, five-seed image-level protocol (AU-ROC mean~$\pm$~std). Source: \textit{FINAL Step~2.4}, \texttt{recovered\_region\_aware\_ws\_cnn/mass\_extensions\_story\_table.csv}.}
...
\begin{table}[h]
\centering
\caption{Region ablation for the WS$\rightarrow$CNN head (five-seed AU-ROC; selected rows). Every configuration sits well above the averaged-WS baseline ($0.726$), but differences among the stronger ones are within seed noise. Source: \texttt{region\_ws\_ablation/region\_ablation\_cnnhead\_results.csv}.}
...
Because the WS-map CNN is a \emph{learned} model trained on only ${\sim}90$ patches, its five-seed held-out AU-ROC still carries split sensitivity ($\pm 0.036$). A group-aware $10$-fold cross-validation diagnostic gives a tighter estimate, AU-ROC $0.938 \pm 0.007$, confirming that the held-out mean is not a lucky split. We report both, and treat the held-out five-seed mean as the headline with the group-CV as the stability check (Section~\ref{sec:discussion}).
...
\begin{table}[h]
\centering
\caption{Mass task --- master comparison across families, five-seed image-level protocol (AU-ROC mean~$\pm$~std), drawn from the FINAL Step~2.1--2.4 outputs. ``Status'' indicates the method's role in the project narrative.}
...
The useful effect was concentrated in the \emph{mass WS-only} setting (Table~\ref{tab:preproc_mass_ws}). Tissue WS did not improve: the current full-preprocessing row remained closest to the Razali tissue WS reference (AU-ROC $0.895$ vs.\ the paper's $0.940$), while raw or lighter variants fell to $0.881$ or $0.812$. CNN-only and CNN/fusion sensitivity checks were also inconclusive: some settings moved AU-ROC closer to the paper and others moved test accuracy closer, but no CNN variant matched both. This is why the remainder of the preprocessing investigation focuses on mass WS and region-aware WS-map methods.
...
\begin{table}[h]
\centering
\caption{Region-aware WS maps $\rightarrow$ CNN head: current full-preprocessed inputs vs.\ masked/no-CLAHE inputs. Both rows use the all-four-region tensor (\emph{full+inner+boundary+context}) and the same five-seed protocol. Source: \texttt{region\_aware\_masked\_no\_clahe\_vs\_current\_comparison.csv}.}
...
\begin{table}[h]
\centering
\caption{Masked/no-CLAHE region ablation for the WS-map CNN head (five-seed protocol). The table is sorted by AU-ROC. Source: \texttt{region\_ablation\_masked\_no\_clahe\_cnnhead\_results.csv}.}
